<?php
/** Tinglados × San Ramón (subservicio; la página principal de la zona es /galpones-prefabricados/san-ramon). */
return [
    'title'       => 'Tinglados en San Ramón – Presupuesto por WhatsApp',
    'description' => 'Tinglados en San Ramón con presupuesto por WhatsApp: techado abierto para maquinaria, rollos y camiones en campos de Ruta 6 y Ruta 12.',
    'h1'          => 'Tinglados en San Ramón: techado abierto para maquinaria y rollos',
    'eyebrow'     => 'Techados para el campo en Canelones',
    'subtitulo'   => 'Cobertizos metálicos para establecimientos rurales del noreste de Canelones. Medimos en el campo y presupuestamos por ítem por WhatsApp.',
    'secciones'   => [
        [
            'h2' => '¿Cómo hacemos un tinglado en San Ramón?',
            'parrafos' => [
                'Galpones Prefabricados hace tinglados en San Ramón para productores que necesitan techar maquinaria, rollos y camiones sin cerrar un galpón. San Ramón está sobre Ruta 6, junto al río Santa Lucía, y la mayoría de los pedidos vienen de campos y chacras de los alrededores. Mandanos por WhatsApp qué querés cubrir y dónde está el predio; coordinamos la visita y te pasamos el presupuesto.',
                'El tinglado rural se define por altura y luz libre: una cosechadora o un camión con acoplado marcan la columna, y la cantidad de rollos marca el largo. Si después necesitás cerrarlo, se agregan laterales sobre la misma estructura. Para un depósito cerrado desde el principio, mirá el <a href="/galpones-prefabricados/san-ramon">galpón prefabricado en San Ramón</a>.',
            ],
        ],
        [
            'h2' => 'Cómo llegamos y qué encontramos en San Ramón',
            'parrafos' => [
                'Llegamos por Ruta 6 desde Montevideo; Ruta 12 conecta con Tala hacia el este. La ciudad tiene casco urbano chico y un entorno de campos ganaderos, tambos y chacras. El último tramo suele ser camino vecinal de balasto: lo relevamos antes de mandar el camión con la estructura, sobre todo después de lluvias.',
                'Los predios son amplios y con lomas suaves. Elegimos con el productor el lugar del tinglado según el drenaje y la maniobra de la maquinaria. Cerca del río Santa Lucía revisamos el nivel del terreno antes de ubicar las bases. Nos piden tinglados en San Ramón para tractores y cosechadoras, para rollos y fardos que se arruinan a la intemperie, y para camiones que cargan en el establecimiento.',
                'También hacemos <a href="/galpones-prefabricados/tala">galpones en Tala</a>, <a href="/galpones-prefabricados/san-bautista">galpones metálicos en San Bautista</a> y <a href="/galpones-prefabricados/ciudad-de-canelones">depósitos en la ciudad de Canelones</a>, por Ruta 6 hacia el sur.',
            ],
        ],
        [
            'h2' => 'Qué define el precio de un tinglado en San Ramón',
            'parrafos' => ['Cada presupuesto se arma por ítem. Lo que más pesa:'],
            'lista' => [
                'Altura libre bajo cabriada, que la fija la máquina o el camión más alto.',
                'Largo y ancho: cuántos rollos o cuántas máquinas van en fila y en profundidad.',
                'Laterales: abierto total, un lado cerrado contra el viento o semicerrado con media chapa.',
                'Bases de hormigón según el suelo del predio; en zona baja pueden ser más profundas.',
                'Acceso del camión con la estructura por camino vecinal y distancia desde Ruta 6.',
                'Permiso de construcción ante la Intendencia de Canelones, también en suelo rural.',
            ],
        ],
        [
            'h2' => 'Galpones para el campo en San Ramón',
            'parrafos' => [
                'Cuando lo que hay que guardar son insumos, granos o animales, el tinglado queda corto y conviene un galpón cerrado con ventilación: mirá <a href="/galpones-agricolas/san-ramon">galpones agrícolas para el campo de San Ramón</a>. Las medidas y pendientes de techo habituales están en la troncal de <a href="/tinglados">estructuras para techado</a>, y todos los servicios de la <a href="/san-ramon">zona de San Ramón</a> en su página.',
            ],
        ],
    ],
    'faq' => [
        ['q' => '¿Cuánta altura necesita un tinglado para una cosechadora en San Ramón?', 'a' => 'La define la máquina más alta que vas a guardar, con margen para entrar y salir. Mandanos el modelo por WhatsApp y calculamos la altura libre bajo cabriada.'],
        ['q' => '¿Se puede hacer el tinglado en un campo con camino de balasto?', 'a' => 'Sí. Relevamos el camino en la visita y coordinamos el ingreso del camión con la estructura para un día en que el acceso esté firme.'],
        ['q' => '¿Los tinglados en San Ramón protegen los rollos de forraje?', 'a' => 'Protege de la lluvia y del sol, que es lo que más los arruina. Si el viento trae agua de un lado, se cierra ese lateral y el resto queda abierto para ventilar.'],
        ['q' => '¿Galpones Prefabricados hace tinglados chicos, para un solo tractor?', 'a' => 'Sí. Se cotiza igual, por ítem: estructura, techo y bases. El tamaño mínimo lo define lo que vas a guardar.'],
    ],
    'actualizado' => '2026-09-24',
];
