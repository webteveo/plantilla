...
2. **`config.php`**: mapear cada respuesta según `references/preguntas.md` § "Dónde va cada respuesta". Dominio también en `.htaccess`. `schema.tipo` de la tabla de `entidades-y-schema.md` §3. Mensaje de WhatsApp `Hola, necesito {servicio} en {zona}` (o el que dio el usuario). Colores: paleta del usuario o una neutra coherente (sin tocar nombres de variables).
...
5. **Contenido por tandas** (por servicio → nivel de zona: hub del departamento → ciudades → barrios). Cada página = un archivo en `data/contenido/` según `references/redaccion.md` (brief de página, mapeo de bloques de la skill de SEO a los campos de la plantilla, reglas de estilo). Si hay herramienta de subagentes, lanzar tandas de ≤ 10 páginas en paralelo pasándole a cada agente: la ruta de `references/redaccion.md`, un resumen de `config.php`, la fila de datos de cada zona (del JSON + respuestas) y la lista de vecinas a enlazar. Sin subagentes: escribir secuencialmente. Después de cada tanda, actualizar `PROGRESO.md`.
